const variableModel = 'Surat Keterangan Wali Nikah'
function edit(id) {
    save_method = 'update';
    $('#form')[0].reset(); 
    $.ajax({
        url: `${base_url}${apiUrl}keterangan-wali-nikah/${id}`,
        type: 'GET',
        dataType: 'JSON',
...
function save() {
    const id = $('#id').val();
    const url = id ? `${base_url}${apiUrl}keterangan-wali-nikah/${id}` : `${base_url}${apiUrl}keterangan-wali-nikah`;
...
function deleteData(id) {
    if (confirm('Anda yakin ingin melakukan operasi ini?')) {
        $.ajax({
            url: `${base_url}${apiUrl}keterangan-wali-nikah/delete/${id}`,
            type: 'GET',
            dataType: 'JSON',
            success: ({ message }) => {
                alert(message);
