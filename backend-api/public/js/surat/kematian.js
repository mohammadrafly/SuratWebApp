const variableModel = 'Surat Kematian'
function edit(id) {
    save_method = 'update';
    $('#form')[0].reset(); 
    $.ajax({
        url: `${base_url}${apiUrl}kematian/${id}`,
        type: 'GET',
        dataType: 'JSON',
        success: function(respond) {
            ...
            $('#tanggal_meninggal').val(tanggal_meninggal);
            $('#alamat_meninggal').val(alamat_meninggal);
...
function save() {
    const id = $('#id').val();
    const url = id ? `${base_url}${apiUrl}kematian/${id}` : `${base_url}${apiUrl}kematian`;
...
function deleteData(id) {
    if (confirm('Anda yakin ingin melakukan operasi ini?')) {
        $.ajax({
            url: `${base_url}${apiUrl}kematian/delete/${id}`,
            type: 'GET',
            dataType: 'JSON',
            success: ({ message }) => {
                alert(message);
